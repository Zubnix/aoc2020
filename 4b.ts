...
cid:294 hcl:#b6652a iyr:2017
  ...
  const rules: Record<string, (input: string) => boolean> = {
    ['byr']: (input) => {
      const inputAsNumber = Number.parseInt(input)
      return input.length === 4 && inputAsNumber >= 1920 && inputAsNumber <= 2002
    },
    ['iyr']: (input) => {
      const inputAsNumber = Number.parseInt(input)
      return input.length === 4 && inputAsNumber >= 2010 && inputAsNumber <= 2020
    },
    ['eyr']: (input) => {
      const inputAsNumber = Number.parseInt(input)
      return input.length === 4 && inputAsNumber >= 2020 && inputAsNumber <= 2030
    },
    ['hgt']: (input) => {
      const numberInCm = input.endsWith('cm')
      const numberInIn = input.endsWith('in')
      if (numberInCm || numberInIn) {
        const inputAsNumber = Number.parseInt(input.slice(0, -2))
        if (numberInCm) {
          return inputAsNumber >= 150 && inputAsNumber <= 193
        }
        return inputAsNumber >= 59 && inputAsNumber <= 76
      }
      return false
    },
    ['hcl']: (input) => {
      const hasHash = input.startsWith('#')
      if (hasHash) {
        const inputWithoutHash = input.slice(1)
        return inputWithoutHash.length === 6 && (inputWithoutHash.match(new RegExp('[0-9]|[a-f]', "g")) || []).length === 6
      }
      return false
    },
    ['ecl']: (input) => {
    ...
    ['pid']: (input) => {
      return input.length === 9 && (input.match(new RegExp('[0-9]', "g")) || []).length === 9
    },
  }

  let count = input.filter(value => {
    return value.includes('byr')
      && value.includes('iyr')
      && value.includes('eyr')
      && value.includes('hgt')
      && value.includes('hcl')
    ...
    const keyValues = value.split(' ').map(value1 => value1.trim().split('\n')).flat().map(value1 => {
      const [key, value] = value1.split(':')
      return { key, value }
    })
